...
public class TextJustify {
    public static String justify(Paint paint,String text,int Width) {
        String tempText;
        String resultText = "";
        ArrayList<String> paraList = new ArrayList<String>();
        paraList = paraBreak(text);
        for(int i = 0; i<paraList.size(); i++) {
            ArrayList<String> lineList=lineBreak("    "+paraList.get(i).trim(),paint,Width);//段首行缩进4个字符
            tempText = TextUtils.join(" ", lineList).replaceFirst("\\s*", "");
            resultText +="    "+tempText.replaceAll("\\s*$", "") + "\n";
        }
        return resultText;
    }
    //分开每个段落
